...
この仕組みを使って、OSSツール **puppet** を作った。

https://github.com/EngineeredReiwa/x-puppet
...
```bash
node puppet.js x tweet "Hello from puppet!"
```
...
```bash
node puppet.js reddit feed 20 javascript
```
...
```bash
node puppet.js discord messages 20
node puppet.js discord send "Hey everyone!"
```
...
puppetの設計で一番こだわったのは「新しいSNSを追加するのが簡単」ということ。
...
**puppet** - Multi-platform social media automation via Chrome DevTools Protocol

GitHub: https://github.com/EngineeredReiwa/x-puppet
